    ...
    sample_ec_key_128bit_t *second_derived_key)
{
    ...
    sample_ret = sample_sha256_update((uint8_t*)&hash_buffer, sizeof(hash_buffer_t),
        sha_context);
    if (sample_ret != SAMPLE_SUCCESS)
    {
    ...
    static_assert(sizeof(sample_ec_key_128bit_t)* 2 == sizeof(sample_sha256_hash_t),
        "structure size mismatch.");
    memcpy_s(first_derived_key, sizeof(sample_ec_key_128bit_t), &key_material,
        sizeof(sample_ec_key_128bit_t));
    memcpy_s(second_derived_key, sizeof(sample_ec_key_128bit_t),
        (uint8_t*)&key_material + sizeof(sample_ec_key_128bit_t),
        sizeof(sample_ec_key_128bit_t));
