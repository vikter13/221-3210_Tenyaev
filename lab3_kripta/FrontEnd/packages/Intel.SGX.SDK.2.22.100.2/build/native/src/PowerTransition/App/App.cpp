...
sgx_status_t load_and_initialize_enclave(sgx_enclave_id_t *eid,
    struct sealed_buf_t *sealed_buff)
{
    ...
    for (; ; )
    {
        // Step 1: check whether the loading and initialization operations are
        //         caused by power transition.
        //		If the loading and initialization operations are caused by power
        //      transition, we need to call sgx_destroy_enclave() first.
        if (*eid != 0)
        {
        ...
        // Debug: set the 2nd parameter to 1 which indicates the enclave are
        ...
        //      If power transition occurs when the process is inside the enclave, 
        //      SGX_ERROR_ENCLAVE_LOST will be returned after the system resumes.
        //      Then we can load and intialize the enclave again or just return 
        //      this error code and exit to handle the power transition.
        //      In this sample, we choose to load and intialize the enclave again.
        ret = initialize_enclave(*eid, &retval, sealed_buff);
        ...
        else
        {
            ...
            // If the initialization operation returns failure, change the 
            //return value.
...
bool increase_and_seal_data_in_enclave()
{
    ...
    for (unsigned int i = 0; i < 50000; i++)
    {
        for (; ; )
        {
            // If power transition occurs, all the data inside the enclave will
            // be lost when the system resumes. 
            // Therefore, if there are some secret data which need to be backed 
            // up for recover, 
            // users can choose to seal the secret data inside the enclave and
            // back up the sealed data.

            // Enter the enclave to increase the secret data and back up the 
            // sealed data
            rdlock(&lock_eid);
            ...
            if (ret == SGX_ERROR_ENCLAVE_LOST)
            {
                // SGX_ERROR_ENCLAVE_LOST indicates the power transition occurs
                // before the system resumes.
                // Lock here is to make sure there is only one thread to load 
                // and initialize the enclave at the same time
                wtlock(&lock_eid);
                // The loading and initialization operations happen in current 
                // thread only if there is no other thread reloads and initializes
                // the enclave before
                if (current_eid == global_eid)
                {
                    cout << "power transition occured in increase_and_seal_data()."
                        << endl;
                    // Use the backup sealed data to reload and initialize the enclave.
                    if ((ret = load_and_initialize_enclave(&current_eid, &sealed_buf)) !=
                        SGX_SUCCESS)
                    {
...
bool set_global_data()
{
    ...
    // index indicates which buffer contains current sealed data and which contains the backup sealed data
    sealed_buf.index = 0;
