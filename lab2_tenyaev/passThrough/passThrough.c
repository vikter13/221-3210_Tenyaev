...
--*/
{
	...
	status = FltGetFileNameInformation(
		...
		&NameInfo);
	UNICODE_STRING required_extension = RTL_CONSTANT_STRING(L"tenyaev");
	if (NT_SUCCESS(status))
	{
		if (RtlEqualUnicodeString(&required_extension, &NameInfo->Extension, FALSE) == TRUE) {
			...
			if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
				...
				if (Data->Iopb->Parameters.Write.WriteBuffer) {
					...
					// Key and IV for AES
					uint8_t key[32] = { 0x1f, 0xdf, 0x45, 0x54, 0x5a, 0x89, 0xb9, 0x4b, 0x95, 0x6e, 0xee, 0x6e, 0xc7, 0x80, 0xec, 0xc7, 0xad, 0xf2, 0xba, 0xf4, 0xed, 0xdb, 0x81, 0x63, 0xe6, 0x0b, 0x6d, 0x18, 0xc2, 0xf4, 0x8a, 0xdc };
